...
def assert_send_message(
    ...
    assert mail.outbox[0].subject == send_message_data["subject"]
    assert mail.outbox[0].body == send_message_data["body"]
    assert mail.outbox[0].from_email == settings.SUPPORT_EMAIL
    assert mail.outbox[0].to == expected_emails
...
def test_cannot_send_message_with_nonexistent_registration_id(api_client, user):
    ...
    assert response.data["detail"] == "Registration nonexistent-id doesn't exist."
