...
def delete_registration(api_client, registration_id):
    delete_url = reverse("registration-detail", kwargs={"pk": registration_id})
    response = api_client.delete(delete_url)

    return response


def assert_delete_registration(api_client, registration_id):
    response = delete_registration(api_client, registration_id)
    assert response.status_code == status.HTTP_204_NO_CONTENT

    detail_url = reverse("registration-detail", kwargs={"pk": registration_id})
    response = api_client.get(detail_url)
    assert response.status_code == status.HTTP_404_NOT_FOUND


@pytest.mark.django_db
def test_registration_delete(api_client, registration, user):
    api_client.force_authenticate(user)

    assert_delete_registration(api_client, registration.id)
