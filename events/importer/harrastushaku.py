...
class HarrastushakuImporter(Importer):
    ...
    def setup(self):
        ...
        self.ahjo_data_source, _ = DataSource.objects.get_or_create(
            id="ahjo", defaults={"name": "Ahjo"}
        )
        self.organization, _ = Organization.objects.get_or_create(
            origin_id="u48040030", data_source=self.ahjo_data_source
        )
        self.tprek_ids = {
            place.origin_id
            for place in Place.objects.filter(data_source=self.tprek_data_source)
        }
    ...
    def import_places(self):
        ...
        self.location_id_to_place_id = self.map_harrastushaku_location_ids_to_tprek_ids(
            locations
        )
    ...
    def map_harrastushaku_location_ids_to_tprek_ids(self, harrastushaku_locations):
        ...
        for harrastushaku_location in harrastushaku_locations:
            ...
            if tprek_place:
                ...
                result[harrastushaku_location_id] = "{}:{}".format(
                    self.data_source.id, harrastushaku_location_id
                )
    ...
    def handle_location(self, location_data):
        ...
        harrastushaku_location_mapped_id = self.location_id_to_place_id.get(
            harrastushaku_location_id
        )
    ...
    def handle_activity(self, activity_data):
        if activity_data.get("active") not in ("1", "K"):
            logger.debug(
                "Skipping inactive activity {}".format(activity_data.get("id"))
            )
            return
    ...
    def create_registration_links(self, activity_data):
        ...
        if (
            activity_data.get("regavailable", 0)
            and "1" in activity_data["regavailable"]
        ):
            ...
            reg_start = (
                activity_data["regstartdate"]
                if isinstance(activity_data["regstartdate"], int)
                else 0
            )
            reg_end = (
                activity_data["regenddate"]
                if isinstance(activity_data["regenddate"], int)
                else 9999999999
            )
            if (
                datetime.utcfromtimestamp(reg_start)
                <= datetime.utcnow()
                <= datetime.utcfromtimestamp(reg_end)
            ):
                return {
                    "fi": {
                        "registration": f"https://harrastushaku.fi/register/{activity_data['id']}"
                    }
                }
        return ""
    ...
    def get_event_data(self, activity_data):
        ...
        audience = self.get_event_audiences_from_ages(
            activity_data
    ...
    def get_organizer(self, activity_data):
        return {
            "fi": clean_text(
                activity_data.get("organiser", ""), strip_newlines=True, parse_html=True
            )
        }
    ...
    def handle_recurring_event(self, event_data, time_tables):
        ...
        if not sub_event_time_ranges:
            raise HarrastushakuException(
                "Erroneous time tables: {}".format(time_tables)
            )
    ...
    def get_event_keywords_from_main_categories(self, activity_data):
        main_category_ids = {
            c.get("maincategory_id") for c in activity_data.get("categories", [])
        }
    ...
    def get_event_keywords_from_search_words(self, activity_data):
        ...
        cleaned_search_words = [
            s.strip().lower() for s in search_words.split(",") if s.strip()
        ]
    ...
    def get_event_languages(self, activity_data):
        ...
        languages = {
            obj
            for code, obj in self.languages.items()
            if obj.name_fi and obj.name_fi in language_text
        }
        return languages
    ...
    def get_organizer_info(self, activity_data):
        org_details = clean_text(
            activity_data.get("organiserdetails", ""),
            strip_newlines=True,
            parse_html=True,
        )
        reg_details = clean_text(
            activity_data.get("regdetails", ""), strip_newlines=True, parse_html=True
        )
        return (
            {"fi": f"{reg_details} {org_details}".strip()}
            if org_details or reg_details
            else ""
        )
    ...
    def get_event_audiences_from_ages(self, activity_data):
        ...
        return {
            self.keywords.get(k_id)
            for k_id in audience_keyword_ids
            if k_id in self.keywords
        }
